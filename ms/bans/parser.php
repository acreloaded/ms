...
// IP bans
if ( $f = @fopen( dirname( __FILE__ ).'/ipbans.txt', 'r' ) )
	while ( ( $buf = readcfg( $f ) ) !== false ) {
		if ( atoipr( $buf, $ipr ) !== false )
			// 1: play, 2: register, 3: all
			$config['sbans'] []= array( $ipr[0], $ipr[1], 3, 'ip is in a proxy range' );
	}
// Nickname Bans
if ( $f = @fopen( dirname( __FILE__ ).'/nickbans.txt', 'r' ) )
	while ( ( $buf = readcfg( $f ) ) !== false ) {
